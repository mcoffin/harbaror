...
import Harbaror.Config (Webhook, webhookRequestHeaders, webhookExclusions)
...
foreign import testExclusion :: String -> Regex -> Foreign -> Boolean
...
doRequest webhook view = do
    ...
    exclusions <- (SM.toUnfoldable <$> webhookExclusions webhook) :: _ (Array (Tuple String Regex))
    let shouldExclude = or $ exclusions <#> \(Tuple query pattern) -> testExclusion query pattern view
    if shouldExclude
       then pure "Excluded"
       else do
